...
- Issue 10: Author click should route through author filter state and avoid overwriting the search term
...
3b. Author click follow-up (Issue 10):
- Update PublicLibrary onAuthorClick to use the author filter state (or a specific authorId filter, if added).
- Do not overwrite the existing search term when clicking an author name.
- If author filter remains Mine/Others only, decide whether author click should set a specific authorId filter or remain a search convenience (document the choice).
...
15. Click an author name on a card - author filter updates without clearing the current search term (Issue 10)

**Cross-page test:**
15. Set filter on Dashboard, navigate to Library, verify same filter applied
16. Sign out and back in - verify filters persist across sessions
  </how-to-verify>
