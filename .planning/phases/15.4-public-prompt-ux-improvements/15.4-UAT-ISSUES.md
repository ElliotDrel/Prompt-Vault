...
|--------|-------|-------------|
| IMPLEMENT NOW | 15 | Must fix before merge |
| CONDITIONAL | 6 | Fix if specific condition applies |
| LATER | 2 | Valid but lower priority |
| IGNORE | 2 | False positives or not applicable |
...
### UAT-041-10: Inconsistent `isOwnPrompt` Prop Naming (MEDIUM)

**File:** `src/components/PromptCard.tsx`
**Lines:** 80-81, 223

**Problem:**
The prop documentation says:
```typescript
/** Whether this prompt is owned by the current user (for visual distinction in Library) */
isOwnPrompt?: boolean;
```

But the styling logic is inverted:
```typescript
isOwnPrompt === false ? 'ring-2 ring-green-500/50 bg-green-50/30' : ''
```

When `isOwnPrompt` is FALSE, the green highlight appears. This is counterintuitive - developers expect `isOwnPrompt=true` to mean "owned".

**Impact:**
- Confusing for future maintainers
- Easy to introduce bugs when using this prop
- Related to documentation mismatch in Comment 1

**Fix Required:**
Either:
1. Rename prop to `isFromOtherUser` or `isNotOwnPrompt` and update all usages
2. OR flip the boolean logic: pass `true` for non-owned and style on `isOwnPrompt === true`

Option 1 recommended for clarity.

**Verification:**
1. Green outline still appears on non-owned prompts in Library
2. Prop name/doc clearly matches behavior
...
**CREATE A GAP CLOSURE PLAN** that addresses all **IMPLEMENT NOW** issues (UAT-041-01 through UAT-041-15).
...
5. **Code cleanup:** UAT-041-04 (unused imports)
6. **Clarity improvements:** UAT-041-10 (prop naming), UAT-041-11 (search normalization)
7. **Security:** UAT-041-12 (trust gate)
8. **Documentation:** UAT-041-13, UAT-041-14, UAT-041-15

Do NOT plan fixes for CONDITIONAL, LATER, or IGNORE items.
