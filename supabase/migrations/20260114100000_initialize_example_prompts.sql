-- Initialize example prompts for new users
-- Creates 4 starter prompts when a user signs up to demonstrate platform features
-- Following the same pattern as initialize_user_settings()

-- Create trigger function to initialize example prompts for new users
...
-- Add trigger to auth.users table (fires after user creation)
CREATE TRIGGER on_auth_user_created_example_prompts
    AFTER INSERT ON auth.users
    FOR EACH ROW
    EXECUTE FUNCTION public.initialize_example_prompts();

-- Add comment for documentation
COMMENT ON FUNCTION public.initialize_example_prompts() IS 'Creates 4 example prompts for new users: Welcome (pinned, 2 versions), Developer, Productivity (with variables), and Image Generation. Runs automatically on user signup.';
