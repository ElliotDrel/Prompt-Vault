...
BEGIN
    ...
    LOOP
        ...
        -- Create version 1 for Welcome (older timestamp - demonstrates version history)
        INSERT INTO public.prompt_versions (prompt_id, user_id, version_number, title, body, variables, created_at)
        VALUES (v_welcome_id, v_user.id, 1, v_welcome_title, v_welcome_body_v1, '[]'::jsonb, v_one_minute_ago);

        -- Create version 2 for Welcome (current timestamp)
        INSERT INTO public.prompt_versions (prompt_id, user_id, version_number, title, body, variables, created_at)
        VALUES (v_welcome_id, v_user.id, 2, v_welcome_title, v_welcome_body_v2, '[]'::jsonb, v_now);

        -- Insert Developer prompt (not pinned)
        INSERT INTO public.prompts (user_id, title, body, variables, is_pinned, times_used, created_at, updated_at)
        VALUES (v_user.id, v_developer_title, v_developer_body, '[]'::jsonb, false, 0, v_now, v_now)
        RETURNING id INTO v_developer_id;

        -- Create version 1 for Developer
        INSERT INTO public.prompt_versions (prompt_id, user_id, version_number, title, body, variables, created_at)
        VALUES (v_developer_id, v_user.id, 1, v_developer_title, v_developer_body, '[]'::jsonb, v_now);

        -- Insert Productivity prompt (with variables)
        INSERT INTO public.prompts (user_id, title, body, variables, is_pinned, times_used, created_at, updated_at)
        VALUES (v_user.id, v_productivity_title, v_productivity_body, v_productivity_vars, false, 0, v_now, v_now)
        RETURNING id INTO v_productivity_id;

        -- Create version 1 for Productivity
        INSERT INTO public.prompt_versions (prompt_id, user_id, version_number, title, body, variables, created_at)
        VALUES (v_productivity_id, v_user.id, 1, v_productivity_title, v_productivity_body, v_productivity_vars, v_now);

        -- Insert Image prompt
        INSERT INTO public.prompts (user_id, title, body, variables, is_pinned, times_used, created_at, updated_at)
        VALUES (v_user.id, v_image_title, v_image_body, '[]'::jsonb, false, 0, v_now, v_now)
        RETURNING id INTO v_image_id;

        -- Create version 1 for Image
        INSERT INTO public.prompt_versions (prompt_id, user_id, version_number, title, body, variables, created_at)
        VALUES (v_image_id, v_user.id, 1, v_image_title, v_image_body, '[]'::jsonb, v_now);

        RAISE NOTICE 'Added 4 example prompts for user %', v_user.id;
    END LOOP;

    RAISE NOTICE 'Backfill complete: Added example prompts to 12 users';
END;
$$;
